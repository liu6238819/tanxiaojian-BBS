...
export default {
	...
	props: {
		...
		autoUpload: {
			type: Boolean,
			default: true
			//default: false//默认改为false,就不会自动上传图片
		},
		// 是否显示toast消息提示
		showTips: {
			type: Boolean,
	...
	methods: {
		...
		// 选择图片
		selectFile() {
			if (this.disabled) return;//是否启用显示组件
				...
				.then(res => {
					...
					res.tempFiles.map((val, index) => {
						...
						if (val.size > maxSize) {
							...
							this.showToast('暂不支持上传超过'+maxSize/1024/1024+'M的图片！');
						} else {
		...
		async uploadFile(index = 0) {
			...
			// 创建上传对象
			var imgUrl = await this.uploadImageOSS(this.lists[index].url, 'userCertificate/',this.currentSchoolId);
			if(!imgUrl){
				uni.showToast({
					title: '上传图片失败，请重新上传',
				});
			}
			this.lists[index].response = imgUrl;
